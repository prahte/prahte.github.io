...
for($i=0; $i<10; $i++) {
    if($_FILES['asset_'.$i.'_new']['name'] != '') {
        ...
        if($_FILES['asset_'.$i.'_new']['size'] <= 1000000) {
...
if($_SESSION['form']['post_title'] == '') { $_SESSION['alert']['fields'] .= ",post_title"; $_SESSION['alert']['message'] .= '<em>You must provide a title for this blog entry.</em><br />'; $errors++; }
if($_SESSION['form']['post_content'] == '') { $_SESSION['alert']['fields'] .= ",post_content"; $_SESSION['alert']['message'] .= '<em>You must provide content for this blog entry.</em><br />'; $errors++; }
if($_SESSION['form']['user_id'] == '') { $_SESSION['alert']['fields'] .= ",user_id"; $_SESSION['alert']['message'] .= '<em>Please select an Entry Author.</em><br />'; $errors++; }

if( !(eregi("([0-9]{2})/([0-9]{2})/([0-9]{4})", $_SESSION['form']['post_day'])) ) {
    $_SESSION['alert']['fields'] .= ',post_day';
    $_SESSION['alert']['message'] .= '<em>Please format the entry date as MM/DD/YYYY.</em><br />';
    $errors++;
}

if( !(eregi("([0-9]{2}):([0-9]{2}) ([APM]{2})", $_SESSION['form']['post_time'])) ) {
    $_SESSION['alert']['fields'] .= ',post_time';
    $_SESSION['alert']['message'] .= '<em>Please format the entry time as HH:MM AM/PM.</em><br />';
    $errors++;
}
