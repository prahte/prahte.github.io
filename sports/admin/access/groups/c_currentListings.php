<?php for($i=0; $i<count($activeListings); $i++) {
	...
	<li>
		...
		<?php if( ($_SESSION['admin']['access_level'] > 1) || $groupAccess > 2 ) { ?>
		<form action="/sports/admin/access/docs/index.php?dID=<?=$files[$i]['id']?>&amp;gID=<?=$_REQUEST['gID']?>" method="post" class="p-border gray-bkgd-fade">
	...
	<?php if( ($_SESSION['admin']['access_level'] > 1) || $groupAccess > 2 ) { ?>
	<h5 class="plus"><a href="/sports/admin/access/docs/index.php?n=1&amp;gID=<?=$_REQUEST['gID']?>">Add a Document</a></h5>
	<?php } ?>
...
if( $groupAccess > 1 || $_SESSION['admin']['access_level'] > 1 ) { ?>
	...
	<a href="/sports/admin/access/members/index.php?uID=<?=$members[$g]['id']?>&amp;gID=<?=$_REQUEST['gID']?>">
	<?php } ?>
	<span class="bold">
